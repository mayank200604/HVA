...
const ChatFAB = () => {
    ...
    useEffect(() => {
        ...
        window.addEventListener('toggleRAGChat', handleToggle);
        return () => window.removeEventListener('toggleRAGChat', handleToggle);
    }, []);
    ...
    return (
        <>
            {/* Chat Window - Positioned based on screen size */}
            <div className={`fixed z-[100] flex flex-col items-end pointer-events-none ${isOpen
                    ? 'top-16 right-4 sm:right-6 lg:bottom-6 lg:top-auto'
                ...
                <AnimatePresence>
                    {isOpen && (
                        <motion.div
                            ...
                            className="pointer-events-auto w-[calc(100vw-2rem)] sm:w-[calc(100vw-3rem)] max-w-[380px] lg:w-[380px] h-[calc(100vh-5rem)] sm:h-[calc(100vh-6rem)] lg:h-[600px] mb-0 lg:mb-4 flex flex-col overflow-hidden rounded-2xl shadow-2xl border border-gray-200/50 dark:border-gray-700/50 backdrop-blur-xl bg-white/95 dark:bg-gray-900/95"
                        >
                            {/* Header */}
            ...
            <div className="hidden lg:block fixed bottom-6 right-6 z-[100] pointer-events-none">
                <button
                    onClick={toggleChat}
                    className={`pointer-events-auto w-14 h-14 rounded-full shadow-xl shadow-blue-500/30 transition-all duration-300 transform hover:scale-110 active:scale-95 flex items-center justify-center backdrop-blur-sm ${isOpen
                    ...
                    aria-label={isOpen ? "Close chat" : "Open career guidance chat"}
                >
                    {isOpen ? (
...
// Export the navbar button component for use in ChatAppPage
export const RAGNavbarButton = () => {
    const location = useLocation();
    const { currentUser } = useAuth();

    if (location.pathname !== "/app" || !currentUser) {
        return null;
    }

    return (
        <button
            className="lg:hidden p-2 -mr-2 rounded-lg hover:bg-slate-800/50 transition-colors"
            aria-label="Open career guidance chat"
            onClick={() => {
                // Trigger the main ChatFAB to open
                const event = new CustomEvent('toggleRAGChat');
